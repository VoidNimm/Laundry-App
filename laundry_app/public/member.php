...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nama'])) {
    $stmt = $pdo->prepare('INSERT INTO tb_member (nama, alamat, jenis_kelamin, tlp) VALUES (?, ?, ?, ?)');
    $stmt->execute([$_POST['nama'], $_POST['alamat'], $_POST['jenis_kelamin'], $_POST['tlp']]);
    header('Location: member.php');
    exit;
}

$members = $pdo->query('SELECT * FROM tb_member ORDER BY id DESC')->fetchAll();
?><!doctype html>
  ...
  <main class="pt-24">
    <div class="max-w-6xl mx-auto px-4">
      <div class="flex items-end justify-between">
        <div>
          <h1 class="phi-h2 font-semibold">Members</h1>
          <p class="text-neutral-400 mt-1 phi-body">Manage your customer directory.</p>
        </div>
      ...
      <section class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="rounded-xl border border-white/10 p-5 md:col-span-1" data-animate="fade-up">
          ...
          <form method="post" action="member.php" class="mt-4 space-y-3 text-sm">
            <label class="block">Nama
              <input name="nama" required class="mt-1 w-full bg-black/20 rounded border border-white/10 p-2">
            </label>
            <label class="block">Alamat
              <textarea name="alamat" class="mt-1 w-full bg-black/20 rounded border border-white/10 p-2"></textarea>
            </label>
            <label class="block">Jenis Kelamin
              <select name="jenis_kelamin" class="mt-1 w-full bg-black/20 rounded border border-white/10 p-2">
                <option value="L">L</option>
                <option value="P">P</option>
              </select>
            </label>
            <label class="block">Telp
              <input name="tlp" class="mt-1 w-full bg-black/20 rounded border border-white/10 p-2">
            </label>
          ...
          <div class="mt-3 overflow-x-auto">
            <table class="min-w-full text-sm">
              <thead class="text-left text-neutral-400">
                <tr><th class="py-2 pr-3">#</th><th class="py-2 pr-3">Nama</th><th class="py-2 pr-3">Telp</th><th class="py-2 pr-3">Jenis Kelamin</th></tr>
              </thead>
              <tbody>
                <?php foreach($members as $m) echo '<tr class="border-t border-white/5"><td class="py-2 pr-3">'.$m['id'].'</td><td class="py-2 pr-3">'.htmlspecialchars($m['nama']).'</td><td class="py-2 pr-3">'.htmlspecialchars($m['tlp']).'</td><td class="py-2 pr-3">'.$m['jenis_kelamin'].'</td></tr>'; ?>
              </tbody>
            </table>
          </div>
        </div>
