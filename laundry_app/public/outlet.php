...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nama'])) {
    $stmt = $pdo->prepare('INSERT INTO tb_outlet (nama, alamat, tlp) VALUES (?, ?, ?)');
    $stmt->execute([$_POST['nama'], $_POST['alamat'], $_POST['tlp']]);
    header('Location: outlet.php');
    exit;
}

$outlets = $pdo->query('SELECT * FROM tb_outlet ORDER BY id DESC')->fetchAll();
?><!doctype html>
  ...
  <main class="pt-24">
    <div class="max-w-6xl mx-auto px-4">
      <div class="flex items-end justify-between">
        <div>
          <h1 class="phi-h2 font-semibold">Outlets</h1>
          <p class="text-neutral-400 mt-1 phi-body">Manage your locations.</p>
        </div>
      ...
      <section class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="rounded-xl border border-white/10 p-5 md:col-span-1" data-animate="fade-up">
          <h2 class="font-semibold">Add Outlet</h2>
          <form method="post" action="outlet.php" class="mt-4 space-y-3 text-sm">
          ...
          <div class="mt-3 overflow-x-auto">
            <table class="min-w-full text-sm">
              <thead class="text-left text-neutral-400">
                <tr><th class="py-2 pr-3">#</th><th class="py-2 pr-3">Nama</th><th class="py-2 pr-3">Telp</th></tr>
              </thead>
              <tbody>
                <?php foreach($outlets as $o) echo '<tr class="border-t border-white/5"><td class="py-2 pr-3">'.$o['id'].'</td><td class="py-2 pr-3">'.htmlspecialchars($o['nama']).'</td><td class="py-2 pr-3">'.htmlspecialchars($o['tlp']).'</td></tr>'; ?>
              </tbody>
            </table>
          </div>
        </div>
